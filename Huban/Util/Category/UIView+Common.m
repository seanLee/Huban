...
#define kTagSizeView 1008
#define kTagIndicatorView 1100
...
- (void)addIndicatorViewWithCenterPoint:(CGPoint)center {
    UIView *indicatorView = [self viewWithTag:kTagIndicatorView];
    if (indicatorView && [indicatorView isKindOfClass:[UIView class]]) {
        indicatorView.hidden = NO;
    } else {
        indicatorView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 10.f, 10.f)];
        indicatorView.backgroundColor = [UIColor colorWithHexString:@"0xe1272f"];
...
- (void)removeIndicatorView {
    ...
    if (subViews && [subViews count] > 0) {
        for (UIView *aView in subViews) {
            if (aView.tag == kTagIndicatorView && [aView isKindOfClass:[UIView class]]) {
                aView.hidden = YES;
            }
        }
    }
...
- (void)addLineLeft:(BOOL)hasLeft andRight:(BOOL)hasRight{
    [self removeViewWithTag:kTagSizeView];
    if (hasLeft) {
        UIView *upView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 0.5f, self.frame.size.height)];
        upView.backgroundColor = [UIColor colorWithHexString:@"0xc8c7cc"];
        upView.tag = kTagSizeView;
        [self addSubview:upView];
    }
    if (hasRight) {
        UIView *upView = [[UIView alloc] initWithFrame:CGRectMake(self.frame.size.width - 0.5f, 0, 0.5f, self.frame.size.height)];
        upView.backgroundColor = [UIColor colorWithHexString:@"0xc8c7cc"];
        upView.tag = kTagSizeView;
        [self addSubview:upView];
    }
...
//        空白数据
        ...
        switch (blankPageType) {
            ...
            case EaseBlankPageTypeTweet://冒泡列表（自己的）
            {
                ...
                break;
            case EaseBlankPageTypeTweetPrivate: {
                imageName = @"blankpage_image_Hi";
                tipStr = @"赶紧发点东西吧!～";
            }
                break;
            case EaseBlankPageTypeTweetPrivateOther: {
                imageName = @"blankpage_image_Hi";
                tipStr = @"暂无信息";
            }
                break;

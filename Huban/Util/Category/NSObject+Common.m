...
- (NSString *)tipFromError:(NSError *)error{
    if (error && error.userInfo) {
        NSString *tipStr;
        if (error.code == -1009) { //如果网络被断开
            return @"请检查您的网络";
        } else if (error.code == 404) {
            return @"服务器错误,请稍后再试";
        } else {
            NSArray *errorArray = error.userInfo[@"subErrors"];
            if (errorArray) {
                tipStr = [errorArray firstObject][@"message"];
            } else {
                tipStr = error.userInfo[@"message"];
            }
            return tipStr;
        }
    }
...
+ (BOOL)saveResponseData:(NSDictionary *)data toPath:(NSString *)requestPath{
    ...
    if (!loginUser) {
        ...
        requestPath = [NSString stringWithFormat:@"%@_%@", loginUser.useruid, requestPath];
    }
...
-(id)handleResponse:(id)responseJSON autoShowError:(BOOL)autoShowError{
    ...
    if (resultCode.intValue != 0) {
        ...
        if (resultCode.intValue == 21) {
            if ([Login isLogin]) { //如果用户在其他设备上登录过该帐号,即退出
                [Login doLogout];
                [((AppDelegate *)[UIApplication sharedApplication].delegate) setupLoginViewController];
            }
            [[NetAPIManager shareManager] request_keepAliveWithBlock:^(id data, NSError *error) {

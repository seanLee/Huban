        ...
        // Add to property name, because it is a type already
            ...
            if ([classType isEqualToString:@"T@\"NSDate\""]) {
                ...
                if ([dict objectForKey:key] != [NSNull null]) {
                    ...
                    if ([classType isEqualToString:@"T@\"NSString\""]) { //字符串默认设置给空串
                        [newObject setValue:@"" forKey:propertyName];
                    } else { //数字则为0
                        [newObject setValue:@0 forKey:propertyName];
                    }
...
//                            日期字符串
                        ...
                        NSObject *obj = [nestedArray[xx] objectForKey:newKey];
                        if ([obj isEqual:[NSNull null]]) { //如果value为"<nsnull>"
                            NSString *type = [nestedObj classOfPropertyNamed:newKey];
                            if ([type isEqualToString:@"NSString"]) {
                                [nestedObj setValue:@"" forKey:tempNewKey];
                            } else if ([type isEqualToString:@"NSNumber"]) {
                                [nestedObj setValue:@0 forKey:tempNewKey];
                            }
                        } else {
                            [nestedObj setValue:obj forKey:tempNewKey];
                        }
                    }
                }
            }
...
+ (NSString *)encodeBase64WithImage:(UIImage *)image {
    NSData *imageData = UIImageJPEGRepresentation(image, 1.f);
    NSString *encodeBase64Str = [NSObject encodeBase64WithData:imageData];

...
- (void)play:(NSString *)file validator:(id)validator {
    ...
    NSError *err;
    //转换arm为wav
    NSString *waveFiele = [AudioAmrUtil convertedAmrFromWave:file]; //首先获取该路径下的wave文件,如果不存在,转换amr To wav
    if (!waveFiele) {
        waveFiele =  [AudioAmrUtil decodeAmrToWave:file];
    }
    NSURL *url = [NSURL fileURLWithPath:waveFiele isDirectory:NO];

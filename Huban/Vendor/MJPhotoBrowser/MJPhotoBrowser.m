...
@property (assign, nonatomic) BOOL responseLongPress;
@end
...
- (void)photoViewLongPress:(MJPhotoView *)photoView {
    if (!_responseLongPress) {
        _responseLongPress = YES;
        @weakify(self);
        if (self.showTranspondAction) {
            [[UIActionSheet bk_actionSheetCustomWithTitle:@"请选择操作方式" buttonTitles:@[@"发送给朋友",@"保存到手机"] destructiveTitle:@"取消" cancelTitle:nil andDidDismissBlock:^(UIActionSheet *sheet, NSInteger index) {
                @strongify(self);
                self.responseLongPress = NO;
                switch (index) {
                    case 0: {
                        [self transpondAction];
                    }
                        break;
                    case 1: {
                        [self savePhoto];
            ...
            [[UIActionSheet bk_actionSheetCustomWithTitle:@"请选择操作方式" buttonTitles:@[@"保存到手机"] destructiveTitle:@"取消" cancelTitle:nil andDidDismissBlock:^(UIActionSheet *sheet, NSInteger index) {
                @strongify(self);
                self.responseLongPress = NO;
                switch (index) {
                    case 0: {
...
#pragma mark - Action
- (void)savePhoto {
    MJPhotoView *photoView = [_visiblePhotoViews allObjects].firstObject;
...
- (void)transpondAction {
    [self hidePhotoBrowser];
    MJPhotoView *photoView = [_visiblePhotoViews allObjects].firstObject;
    MJPhoto *photo = photoView.photo;
    if (photo.image) {
        if (self.transpondBlock) {
            self.transpondBlock(photo.image);
        }
    }
}

...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self loadAssetsGroupsWithTypes:self.groupTypes
                         completion:^(NSArray *assetsGroups) {
                             ...
                             if (self.firstShow) {
                                 self.firstShow = NO;
                                 [self pushToCollectionVCWithIndex:0 animated:NO];
                             }
                         }];
    ...
    for (NSURL *selectedAssetURL in self.selectedAssetURLs) {
        ...
        void (^selectAsset)(ALAsset *) = ^(ALAsset *asset) {
            if (asset) {
                ...
                [images addObject:[UIImage fullScreenImageALAsset:asset]];
            }
            // Check if the loading finished
            if (tryCount == weakSelf.selectedAssetURLs.count) {
                ...
                if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(qb_imagePickerController:didSelectImages:)]) {
                    [weakSelf.delegate qb_imagePickerController:weakSelf didSelectImages:[images copy]];
                }
            }
        };
...
{
    [self pushToCollectionVCWithIndex:indexPath.row animated:YES];
}

- (void)pushToCollectionVCWithIndex:(NSInteger)index animated:(BOOL)animated
{
